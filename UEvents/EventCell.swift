...
class EventCell:UITableViewCell{
    ...
    override init(style: UITableViewCellStyle, reuseIdentifier: String!){
        ...
        eventTitle.frame = CGRectMake(5, 75, UIScreen.mainScreen().bounds.size.width - eventTextPadding, 40)
        ...
        clock.frame = CGRectMake(5, 120, clock.image.size.width/1.5, clock.image.size.height/1.5)
        
        eventWhen.textColor = appearanceController.colorWithHexString(colors["Default"]!["Secondary"]!)
        eventWhen.frame = CGRectMake(clock.frame.width+eventTextPadding, 115, UIScreen.mainScreen().bounds.size.width/4, 20)
        eventWhen.font = UIFont(name: "HelveticaNeue", size: 15.0)
        
        location.frame = CGRectMake(5+eventWhen.frame.width+eventTextPadding, 120, location.image.size.width/1.5, location.image.size.height/1.5)
        
        eventWhere.textColor = appearanceController.colorWithHexString(colors["Default"]!["Secondary"]!)
        eventWhere.frame = CGRectMake(eventWhen.frame.width+location.frame.width+(eventTextPadding * 2), 115, UIScreen.mainScreen().bounds.size.width - (eventWhere.frame.origin.x * 2), 20)
    ...
    override func layoutSubviews() {
        self.imageView.frame = CGRectMake(0, 0, UIScreen.mainScreen().bounds.size.width, 140)
    }
}
